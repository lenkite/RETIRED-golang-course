// samples/range/range.go
package main

import "fmt"

func main() {
	//Here we use range to sum the numbers in a slice. Arrays work like
	//this too.
	nums := []int{2, 3, 4}
	sum := 0
	...
	//is the starting byte index of the rune and the second the rune
	//itself.
	fmt.Println("Iterating over string with ascii chars")
	for i, c := range "go" {
		fmt.Println(i, c)
	}

	fmt.Println("Iterating over string with chinese chars")
